...
### Fixed
- **Временная ошибка доступа к облаку больше не стирает очередь телеметрии.** Проверка
  пути обработки HTTP-ответов выявила, что любой `4xx` считался окончательным: при
  истёкшем ключе (`401`/`403`), неверном endpoint (`404`), таймауте (`408`) или rate
  limit (`429`) накопленные сэмплы помечались отправленными, хотя сами payload'ы были
  корректны. Эти ответы теперь оставляют строки в локальной очереди для повторной
  отправки после исправления внешнего условия. Ошибки содержимого (`400`, `413`,
  `415`, `422`) по-прежнему завершают только неотправляемую строку, чтобы она не
  блокировала FIFO.
- **SOC снова доходит до облака после обновления di+ до 2.0.** На машине с di+
  2.0.0b1 ответ `getDiPars` содержит дробное значение, например `SOC:76.1`; прежний
  целочисленный разбор превращал его в `null`, поэтому SOC исчезал из каждого
  отправляемого сэмпла. Разбор теперь сохраняет дробную точность для облака, а
  локальные потребители с целыми процентами получают округлённое совместимое значение.

### Changed
- Классификация ответа cloud ingest вынесена в чистую функцию и покрыта регрессионными
  тестами для успешного ответа, временных ошибок и окончательных отказов payload'а.
- Числовой разбор di+ теперь принимает десятичную точку и запятую и безопасно
  отбрасывает неподставленные placeholders параметров, недоступных в di+ 2.0.

## [0.5.2] - 2026-08-10

### Added
- **Окно живого статуса после парковки без зарядки.** Когда демон сам видит переход
  `powerState >= 1 → 0`, он удерживает wakelock ещё **30 минут**, чтобы сразу после
  парковки телеметрия и команды оставались отзывчивыми. Окно одноразовое и не
  продлевается: запуск демона на уже выключенной машине, периодические пробуждения
  или неполный ответ di+ не создают новый расход 12-вольтовой батареи. При следующем
  включении автомобиля бюджет сразу сбрасывается.
- В компактный parked payload добавлено давление всех четырёх шин, когда di+ его
  предоставляет: на стоянке актуальные значения теперь не теряются из live-снимка.

### Changed
- Документация Di+ сверена с официальным API-каталогом: для параметров явно отмечены
  подтверждённые единицы, state-значения и неизвестные поля вместо предположений.

### Fixed
- **Телеметрия снова уходит в облако (регрессия с 7 августа).** Запрос
  `SettingsDao.setLastKnownSoc` использовал форму `INSERT ... ON CONFLICT ... DO UPDATE`,
  которая требует SQLite 3.24+. Головное устройство DiLink рапортует Android 10, но несёт
  более старую SQLite, чем сток, и не компилирует этот синтаксис:
  `near "ON": syntax error (code 1 SQLITE_ERROR)`. Исключение возникало на **каждом** тике
  опроса 1 Гц и уводило итерацию в общий `catch` — вместе с ней пропадали и выгрузка в
  облако, и маяк живости. Демон видел протухший маяк и забирал поток себе, поэтому в облаке
  вместо 1 Гц оставались только редкие parked-сэмплы демона (~90 строк в сутки вместо ~10000),
  а ~170 км пробега не записались вовсе. Запрос переведён на `INSERT OR REPLACE` —
  эквивалентно для таблицы `settings` вида (key, value) и совместимо со старой SQLite.
- **Сбой записи SOC больше не обрывает весь цикл опроса.** Сохранение baseline SOC
  (`saveLastKnownSoc`) стоит выше по потоку, чем отправка телеметрии в облако, и любое
  исключение из Room/SQL уводило всю итерацию в общий `catch` — вместе с ней пропадали
  и облачная выгрузка, и маяк живости приложения. Теперь запись SOC обёрнута в
  собственный `try/catch`: потеря одной строки SOC не стоит потери потока 1 Гц,
  а следующий тик повторит попытку.
- **Маяк живости пишется до `enqueue`, а не после.** Маяк утверждает «цикл опроса
  приложения ещё крутится», и это верно независимо от судьбы конкретной отправки.
  При старом порядке одно исключение из очереди или настроек гасило маяк, демон видел
  протухшую метку и молча забирал поток телеметрии себе — на облаке это выглядело как
  падение с 1 Гц до редких parked-сэмплов демона.
