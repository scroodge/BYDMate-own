...
- **Быстрый статус и при выключенной машине (демон).** `CommandDaemon` строит свой payload
  ...
  - в быстром режиме шлёт статус, **всегда `live_only`** — иначе во время зарядки проверка
    «ничего не изменилось» ложна и на каждую отправку писалась бы строка истории;
  - ведёт **отдельный таймер** для обычной 60-секундной истории, чтобы быстрый режим её
    не вытеснял;
  - **опрос команд вынесен в отдельный поток**, а сон цикла статуса считается «по
    расписанию», а не «после работы». Интервал отправки никогда не был узким местом:
    раньше один цикл делал DiPars → POST телеметрии → опрос команд последовательно, поэтому
    каждый сетевой вызов входил в период статуса. Замеры на машине: **8–9 с** сначала,
    **~5 с** после ускорения пробуждения и **~3 с** (24 отправки подряд с интервалом 2–4 с)
    только после обеих правок — потока и `pacedSleepMs`, который вычитает потраченное время
    (сон в 3 с *после* 2 с работы даёт период 5 с). История при этом идёт своим ритмом.
  - опрос команд **вернулся к спокойным 6 с** даже при открытом живом экране: он больше не
    лежит на критическом пути статуса, а разрешение живёт дольше одного опроса.
- **Юнит-тесты `CommandDaemon`** (8 шт., `CommandDaemonTest`). Решения о том, когда и
  как отправлять, вынесены из цикла в чистые функции (`planPush`, `PushPlan.liveOnly`,
  `loopSleepMs`) — сам цикл из теста не завести, он навсегда блокируется на DiPars, HTTP
  и `Thread.sleep`. Каждый тест закрывает реальную ошибку этого дня: строка истории каждые
  3 с при зарядке, пропажа истории при долгом просмотре, потолок цикла в 8–9 с.
