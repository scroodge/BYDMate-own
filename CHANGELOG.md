...
- **Офлайн-буфер получил динамический предел до 4 ГиБ с удалённым управлением.** Размер
  ...
  восстановления связи. По умолчанию продолжает действовать защита в 1000 строк до
  включения увеличенного лимита удалённой настройкой.
...
- **Телеметрия di+ сохраняет точное дробное значение SOC.** JSON-поля статуса и
  телеметрии теперь передают `socPrecise` под совместимым ключом `soc`, не меняя
  целочисленный `DiParsData.soc` для существующих локальных потребителей.
