...
> `v0.5.4` wireless-ADB установлен на `way` и проверен 2026-08-26: `versionCode 341`,
> 9a/9b/9c зелёные. Первый post-install app-сэмпл получен в 10:48:38.703 +03 с
...
- **Один общий построитель telemetry-payload вместо трёх (B-19).** `CloudTelemetryPayload`,
  ...
  ревью до релиза, восстановлено аддитивно (ADR-0003-safe). **Полностью верифицировано**:
  post-install smoke-check (9a/9b) чист; daemon-origin окно поймано постфактум в логе
  демона на ночной стоянке 23→24.09 (21:14–09:33, 12+ часов без app, 694 успешных пуша,
  0 ошибок) и сверено напрямую в production (`bydmate_telemetry_samples`,
  `bydmate_live_snapshots`) — восстановленные поля несут реальные значения, включая
  функционально значимый `stall_sentry_mode`. См. [`project-notes.md`](docs/project-notes.md)
  за 2026-09-24.

## [0.5.5] - 2026-09-09

### Added
- **Автономный демон теперь переживает потерю сети без потери телеметрии.** Сэмплы,
  собранные при остановленном приложении, сначала попадают в устойчивый дисковый spool,
  а после возврата приложения импортируются в общую Room-очередь с исходным
  `device_time` и стабильным `sampleId`. Повторный импорт и повторная отправка безопасны,
  а сохранённые backoff и расписание drain не дают накопившемуся потоку обрушиться на
  сервер сразу после восстановления связи.
- **Добавлен учёт фактического объёма и возраста очереди для следующего этапа настройки
  офлайн-буфера.** Room-схема v18 хранит `payloadBytes`, `capturedAt`, `origin` и
  `compactionTier`, а также агрегированную метаинформацию и индексы для оценки базы,
  WAL и свободного места. Старые строки дополняются ограниченными возобновляемыми
  пакетами после открытия базы.
- **Офлайн-буфер получил динамический предел до 4 ГиБ с удалённым управлением.** Размер
  рассчитывается с обязательным резервом файловой системы, на устройствах с нехваткой
  места минимальный предел не выделяется принудительно, а сжатие и удаление затрагивают
  сначала самые старые компактные интервалы, сохраняя новые сэмплы и границу
  восстановления связи. По умолчанию продолжает действовать защита в 1000 строк, а
  увеличенный лимит включается удалённо поэтапным cohort ramp, чтобы рост хранилища
  можно было безопасно наблюдать и откатывать.
- **di+ стал обязательной и явно проверяемой зависимостью онбординга.** Мастер проверяет
  наличие, первый запуск и версию di+, блокирует продолжение с понятным объяснением,
  а для версий ниже 2.0 перечисляет недоступные `vehicleSegments` и
  `chargingSessions`, не встраивая сторонний APK и не запрашивая право установки.
- **Android CI запускает тесты и debug-сборку для изменений приложения.** Проверка
  воспроизводимо ловит ошибки компиляции, unit-тестов и сборки APK до объединения PR.

### Fixed
- **Состояние зарядного пистолета из autoservice теперь сохраняется и в телеметрии
  автономного демона.** Раньше прямое чтение работало, но поле терялось при
  сериализации daemon payload, поэтому в облаке `autoservice_gun_state` оставался
  пустым именно на автономном пути.
- **Все пути одинаково определяют состояние зарядки.** Общая классификация учитывает
  явный `is_charging`, состояние пистолета autoservice/di+ и статус зарядки, устраняя
  расхождения между live-снимком, историей, поездками и почасовыми агрегатами.
- **Телеметрия di+ сохраняет точное дробное значение SOC во foreground и автономном
  демоне.** JSON-поля статуса, телеметрии и daemon payload теперь передают
  `socPrecise` под совместимым ключом `soc`, не меняя целочисленный `DiParsData.soc`
  для существующих локальных потребителей.

### Changed
- **0.5.4 был только внутренней проверочной сборкой и никогда не публиковался как
  публичный GitHub Release.** Публичный выпуск после 0.5.3 начинается с 0.5.5 и включает
  как изменения внутренней 0.5.4, так и перечисленные выше этапы устойчивой телеметрии.

## [0.5.4] - 2026-08-26

### Changed
- **Отключённые удалённые команды больше не создают круглосуточный опрос каждые 6
  секунд.** Этот цикл давал около 14 400 обращений к облачной функции на автомобиль в
  сутки, хотя при приостановленных командах очередь всегда была пустой. Клиент теперь
  читает серверный флаг `commands_enabled`: при `false` оба опросчика переходят на один
  запрос каждые 5 минут, но не останавливаются полностью, чтобы самостоятельно узнать о
  возврате команд. Отсутствующий флаг считается `true` для совместимости со старыми
  серверами, а единая политика каденса не даёт приложению и автономному демону разойтись.
