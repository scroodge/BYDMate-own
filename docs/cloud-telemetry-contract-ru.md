...
- Авто движется: sample примерно раз в 60 секунд.
- Авто стоит: heartbeat примерно раз в 5 минут.
- Зарядка: sample примерно раз в 1 секунду.
- Изменение состояния движения/зарядки также добавляет sample.

Правила отправки:

- Во время зарядки samples копятся и отправляются batch'ем примерно раз в
  минуту.
- В обычном режиме flush interval берется из настроек, допустимый диапазон
  `5..300` секунд, default `60`.
- Максимальный batch size: `120` samples.
