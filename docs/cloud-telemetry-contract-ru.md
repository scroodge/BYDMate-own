...
Локальная БД — источник истины для телеметрии: строка уходит из очереди отправки только после
подтверждения сервером либо после отказа по **содержимому** payload'а.

HTTP `401`, `403`, `404`, `408`, `429` — **retryable**: записи остаются в очереди. Эти коды
описывают условия запроса (ключ, endpoint, rate limit), а не содержимое; сэмплы остаются
валидными и уйдут, когда оператор поправит настройки или лимит спадёт. Раньше весь `4xx` был
non-retryable, и авто с истёкшим ключом уничтожало историю на ходу — см.
`CloudTelemetryClient.RETRYABLE_CLIENT_CODES`.

Остальной `4xx` (`400`, `413`, `415`, `422`) — non-retryable: сервер отверг именно это тело,
retry ничего не изменит. Строка помечается `sentAt` (чтобы одна неотправляемая запись не
блокировала FIFO), но `lastError != null` отличает её от подтверждённой, и payload остаётся
читаемым в `cloud_sync_queue` до prune.
