...
| **Backlog drain** (>15 unsent) | Historical queue batches | Up to **300** samples; persisted one-token bucket refills every **2 s**; retryable failures use persisted full-jitter exponential backoff and honor `Retry-After` | `flushPending()` |
