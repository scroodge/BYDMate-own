...
- **Текущая версия:** `0.4.7` (`versionCode 332`), релиз 2026-07-08.
...
**Действие:** собрать эти изменения в тег `0.4.8` (см. skill `release-apk`).
...
> заданы явно. Отметьте нужное — распишу в задачи.

- 🧭 **Баннер «доступно обновление»** — таблица-каталог `mate_app_releases` заведена
  (v0.3.9.4), но статус самого in-app баннера в APK не подтверждён. Проверить/докрутить.
- 🧭 **Непрерывность истории при переименовании авто** — таблицы телеметрии в Supabase
  хранят `vehicle_id` как часть ключа, поэтому история до/после rename разрывается.
  Батч-фикс `7b37366` устранил *потерю* данных, но *слияние* истории — отдельная задача
  на стороне VoltFlow/Supabase.
- 🧭 **Экспорт/расширенная аналитика авто** — база под это есть (regen/traction kWh,
  часовые агрегаты); оценить продуктовые пробелы в UI.
